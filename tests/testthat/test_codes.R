...
test_that("Partial ICNARC codes are verified correctly", {
  expect_true(verify_icnarc("1.1.1"))
  expect_warning(verify_icnarc("1.1.1.NA.NA"))
  expect_true(
    suppressWarnings(
      verify_icnarc("1.1.1.NA.NA")
      )
    )
  expect_false(verify_icnarc("0.0.0"))
  expect_warning(verify_icnarc("NA.NA.NA"))
  expect_false(
    suppressWarnings(
      verify_icnarc("NA.NA.NA")
      )
  )
})
...
test_that("nhs numbers validate correctly", {
  ...
  expect_equal(validate_nhs(c("6744424270", "0083041737", "2401060723")),
               c(TRUE, TRUE, TRUE))
  expect_equal(validate_nhs("1234567890"), FALSE)
  expect_equal(validate_nhs(c("6744424270", "1234567890")), c(TRUE, FALSE))
  expect_equal(validate_nhs(c("6744424270", "0083041737", "1234567890")),
               c(TRUE, TRUE, FALSE))
})
