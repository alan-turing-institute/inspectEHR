  ...
  df <- df %>%
    mutate(
      epi_end_dttm = case_when(
        ...
        ~ .data$src_end_dttm,
        .data$outcome == "D" & !is.na(.data$death_dttm)
        & (.data$bsd == 0 | is.na(.data$bsd))
        ~ .data$death_dttm,
...
characterise_spells <- function(df = NULL, minutes = 30) {
  df %>%
    ...
    mutate(time_out = .data$epi_start_dttm[-1] %>%
             difftime(
               .data$epi_end_dttm[-length(.data$epi_end_dttm)],
               units = "mins") %>%
             as.integer() %>%
             c(NA)) %>%
...
#' listed
#'
#' @param reference reference table generated by \code{\link{make_reference}}
