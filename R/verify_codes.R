...
#'
#' This function will validate numbers according to the checksum process
...
#' Verify ICNARC Diagnostic Code
#'
#' Verifies that a code conforms to the ICNARC Diagnostic code specification.
#' This is a 5 level hierarchical code that follows a pattern such as
#' ##.##.##.##.##. A code need only supply levels 1:3 to be valid.
#'
...
verify_icnarc <- function(icnarc_code = NULL) {
  if (is.null(icnarc_code)) {
    abort("Please provide an ICNARC code")
  }
  if (class(icnarc_code) != "character") {
    abort("Please enter ICNARC code as characters")
  }
  icnarc <- str_split(icnarc_code, pattern = "[.|/-]") %>%
    ...
    map(function(x) {
      if (!(length(x) %in% 3:5)) {
        ...
        pos_b <- x[2] %in% c(1:12)
        pos_c <- x[3] %in% c(1:13)
        pos_d <- x[4] %in% c(1:54) | is.na(x[4])
        pos_e <- x[5] %in% c(1:17) | is.na(x[5])
